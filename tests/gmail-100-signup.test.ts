...
describe('100 Synthetic Gmail / Email Sign Up Stress Test (No Phone Number)', () => {
  ...
  // Run 100 consecutive synthetic Gmail signups without needing any phone number
  const TOTAL_TEST_ACCOUNTS = 100;
  ...
  test(`Batch create ${TOTAL_TEST_ACCOUNTS} synthetic Gmail accounts without phone numbers`, async () => {
    ...
    for (let i = 1; i <= TOTAL_TEST_ACCOUNTS; i++) {
      ...
      assert.equal(signupData.profile.full_name, fullName, `Name mismatch for ${email}`);
      const token = signupData.session.access_token;

      // 2. Complete Donor Profile (Blood Group & Pincode) without being blocked by phone number
