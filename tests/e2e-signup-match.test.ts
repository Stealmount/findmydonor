...
describe('E2E: Donor and Requester Signup & Match', () => {
  ...
  test('Check if Match is created', async () => {
    ...
    // In our implementation, matching runs on request creation, but sometimes we need a small delay
    ...
    assert.equal(res.status, 200);
    const dashboard = await res.json() as any;
    
    assert.ok(dashboard.matches.length > 0, 'Should find at least 1 match');
    const ourMatch = dashboard.matches.find((m: any) => m.donor_id === donorId);
    assert.ok(ourMatch, 'Match should be with our donor');
  ...
  test('Donor Approves Match', async () => {
    ...
    const reqRes = await fetch(`${BASE}/api/hospital/dashboard`, {
      headers: { 'Authorization': `Bearer ${requesterToken}` }
    });
    const data = await reqRes.json() as any;
    const myMatches = data.matches.filter((m: any) => m.request_id === requestId);
