...
export function AuthHub({ initialMode = 'signin', initialIntent = 'donor', onLoginSuccessDonor, onLoginSuccessRequester, onLoginSuccessInstitution }: AuthHubProps) {
 ...
 loading, error, setError, infoMessage, setInfoMessage,
 handleGoogle, handleGooglePhoneSubmit,
 } = F;

 const card = 'rounded-3xl bg-white/95 border border-ink-200/80 p-6 sm:p-8';
 ...
 const btnPrimary = 'mt-6 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-blood-600 to-blood-700 text-sm font-bold text-white disabled:opacity-50 cursor-pointer';
 const btnGoogle = 'h-12 w-full rounded-lg px-6 py-3 bg-[#4285F4] hover:bg-[#3367D6] text-sm font-bold text-white transition cursor-pointer';
 ...
 <div className="mx-auto mb-4 grid h-14 w-14 place-items-center rounded-2xl blood-drop-gradient"><Heart className="h-7 w-7 fill-white text-white" /></div>
 ...
 : (isHi ? 'एक Google पहचान। आवश्यकता पड़ने पर दोनों भूमिकाएँ।' : 'Sign in with Google. Choose both roles whenever needed.')
 }
 </p>
 {signupStep !== 'google-phone' && <div className="mt-6 inline-flex rounded-2xl border border-ink-200 bg-ink-100 p-1.5">
 {(['signin', 'signup'] as const).map(item => <button id={`auth-mode-${item}`} key={item} type="button" onClick={() => { setMode(item); setError(''); setInfoMessage(''); }} className={`rounded-xl px-6 py-2.5 text-sm font-bold transition cursor-pointer ${mode === item ? 'bg-white text-ink-900' : 'text-ink-600'}`}>{item === 'signin' ? (isHi ? 'साइन इन' : 'Sign in') : (isHi ? 'खाता बनाएं' : 'Create account')}</button>)}
 </div>}
 </header>
 ...
 {/* ─── GOOGLE SIGN-IN (main) ──────────────────────────── */}
 {signupStep !== 'google-phone' && (
 <motion.div key="google-signin" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={card}>
 <button id="google-btn" type="button" onClick={handleGoogle} disabled={loading} className={btnGoogle}>
