...
import { LayoutDashboard, Radio, History, Tent, Users, BarChart3, Bell, Shield, ClipboardList } from 'lucide-react';

export type HospitalView = 'dashboard' | 'requests' | 'donors' | 'live' | 'history' | 'camps' | 'stats';
...
export function HospitalSidebar({ activeView, onNavigate, showCamps, pendingReplies, lowStockCount, isHi }: HospitalSidebarProps) {
  ...
  return (
    <aside className="hidden md:flex w-64 shrink-0 flex-col border-r border-ink-800 bg-ink-950/60 relative z-10">
    {/* Brand */}
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-[13px] font-semibold transition-all cursor-pointer ${
                    active
                      ? 'bg-blood-600/15 text-blood-400 border border-blood-500/25'
