            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
              {bloodTypes.map(type => {
                ...
                const cardStyle = isCritical
                  ? 'border-red-500/45 bg-red-950/20 shadow-[0_0_15px_rgba(239,68,68,0.08)]'
            ...
            <div className="h-32 w-full flex items-end justify-between px-2">
              {[40, 70, 45, 90, 60, 30, 80].map((h, i) => (
                <div key={i} className="w-[10%] bg-ink-850 rounded-t-sm relative" style={{ height: '100%' }}>
                  <div className={`absolute bottom-0 w-full rounded-t-sm transition-all duration-1000 ${i === 3 ? 'bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.5)]' : 'bg-ink-600'}`}
                    style={{ height: `${h}%` }} />
                </div>
              ))}
            </div>
