...
export function HospitalDashboard({ hospital, onLogout }: HospitalDashboardProps) {
 ...
 <LogOut className="w-4 h-4" />
      ...
      <div className="px-3 py-3 flex gap-1 overflow-x-auto">
        {(['dashboard', 'requests', 'donors', 'live', 'history', ...(showCamps ? ['camps'] : []), 'stats'] as HospitalView[]).map(v => (
          <button key={v} onClick={() => setActiveView(v)}
            ...
            {isHi
              ? (v === 'dashboard' ? 'डैशबोर्ड' : v === 'requests' ? 'अनुरोध' : v === 'donors' ? 'दाता' : v === 'live' ? 'लाइव' : v === 'history' ? 'इतिहास' : v === 'stats' ? 'आंकड़े' : 'शिविर')
              : (v === 'dashboard' ? 'Dashboard' : v === 'requests' ? 'Requests' : v === 'donors' ? 'Donors' : v === 'live' ? 'Live' : v === 'history' ? 'History' : v === 'stats' ? 'Stats' : 'Camps')}
          </button>
        ))}
      </div>
 </div>
