...
export default function MatchList({ matches, requests, currentUser, loadingMatchId, onRespond }: MatchListProps) {
 ...
 isApproved ? 'bg-white border-none ring-0' :
 ...
 <span className={`text-[11px] font-semibold ${isApproved ? 'text-emerald-600' : 'text-white/90'}`}>
  ...
  isDeclined ? (isHi ? 'आपने अस्वीकार किया' : 'Declined — thank you') :
 ...
 disabled={loadingMatchId === match.id}
 className="rounded-full bg-white text-blood-700 py-3 text-[12.5px] font-semibold flex items-center justify-center gap-2 hover:bg-white/90 transition-all cursor-pointer"
 >
 ...
 {isApproved && (() => {
  ...
  <div className="mt-5 pt-5 border-t border-ink-100 space-y-4">
   ...
   <p className="text-xs font-bold text-emerald-800 mb-1">{isHi ? 'आपने स्वीकार कर लिया!' : "You've accepted!"}</p>
   <p className="text-xs text-emerald-700">{isHi ? 'अनुरोधकर्ता को सूचित कर दिया गया है। कृपया अस्पताल पर जाएं:' : 'The requester has been notified. Please head to:'}</p>
   <p className="text-sm font-bold text-emerald-900 mt-1">{req.hospital_name}, {req.hospital_area}, {req.hospital_city}</p>
   </div>
 ...
 className="w-full rounded-2xl bg-blood-600 hover:bg-blood-700 text-white py-3.5 px-4 text-xs font-semibold flex items-center justify-center gap-2 transition-all"
 >
