...
export default function ContactInfoBanner({ phone, whatsappPhone, onSaved }: ContactInfoBannerProps) {
 ...
 setSaving(false);
  ...
  className="relative rounded-2xl bg-amber-50 border border-amber-200 p-4 mb-6 animate-in slide-in-from-top-2 duration-300"
  role="region"
  ...
  <div>
  <p className="text-sm font-bold text-amber-900">Add your phone &amp; WhatsApp number</p>
  ...
  <span className="flex-shrink-0 text-xs font-semibold text-amber-700 w-16 text-right">Phone</span>
  <div className="flex flex-1 items-center rounded-xl bg-white border border-amber-200 px-3 h-9 gap-1.5">
  <span className="text-xs text-amber-600 font-mono">+91</span>
  <input
  ...
  className="flex-1 bg-transparent text-xs text-ink-900 placeholder:text-ink-400 outline-none min-w-0"
  />
  </div>
  </div>
  ...
  {/* WhatsApp row */}
  <div className="flex items-center gap-2">
  <span className="flex-shrink-0 text-xs font-semibold text-amber-700 w-16 text-right">WhatsApp</span>
  <div className="flex flex-1 items-center rounded-xl bg-white border border-amber-200 px-3 h-9 gap-1.5">
  <span className="text-xs text-amber-600 font-mono">+91</span>
  <input
  ...
  {error && (
  <p id="contact-banner-error" className="text-xs text-red-600 pl-[4.5rem]">{error}</p>
  )}
  ...
  type="button"
  onClick={handleDismiss}
  className="text-xs text-amber-600 hover:text-amber-800 transition-colors px-3 py-1.5 cursor-pointer"
  >
  Remind me later
  </button>
  <button
  ...
  className="inline-flex items-center gap-1.5 rounded-xl bg-amber-500 hover:bg-amber-600 disabled:opacity-50 disabled:cursor-not-allowed text-xs font-bold text-white px-4 py-1.5 transition-colors cursor-pointer"
  >
  {saving ? (
  <>
