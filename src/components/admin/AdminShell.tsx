...
export type AdminTab =
  | 'overview' | 'donors' | 'requesters' | 'requests' | 'matches'
...
export default function AdminShell({
    ...
    return (
      <button
        ...
        className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl text-[13px] font-medium transition-all cursor-pointer ${
          active
            ? 'bg-blood-600/15 text-white border border-blood-500/30 shadow-[inset_0_0_20px_rgba(244,63,87,0.06)]'
  ...
  const rail = (
    <aside className="w-full md:w-64 bg-ink-950/90 backdrop-blur-xl border-r border-ink-800 flex flex-col shrink-0 h-full">
      <div className="p-5 space-y-1">
        <div className="flex items-center gap-3 px-2 py-1">
          <div className="w-9 h-9 rounded-xl bg-blood-600 flex items-center justify-center text-white shadow-[0_0_20px_rgba(244,63,87,0.35)]">
    ...
    <div className="min-h-screen bg-ink-950 text-white font-sans flex flex-col md:flex-row antialiased relative">
      ...
      <div className="md:hidden fixed top-4 left-4 z-[45]">
        <button onClick={() => setMobileOpen(v => !v)} aria-label={isHi ? 'मेनू' : 'Menu'}
          className="p-2.5 rounded-xl bg-ink-900 border border-ink-700 text-white shadow-premium-lg cursor-pointer">
      ...
      <main className="relative z-10 flex-1 flex flex-col min-w-0 h-screen md:overflow-hidden">
        <header className="sticky top-0 z-20 bg-ink-950/70 backdrop-blur-xl border-b border-ink-800 px-4 md:px-6 py-3 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={onSweep}
              className="px-3 py-2 bg-blood-600 hover:bg-blood-500 text-white rounded-xl text-xs font-bold transition cursor-pointer flex items-center gap-1.5 shadow-[0_0_16px_rgba(244,63,87,0.3)]">
            ...
            <div className="flex items-center rounded-full bg-ink-900 p-0.5 border border-ink-800">
              <button onClick={() => setLanguage('EN')}
                className={`rounded-full px-2.5 py-1 text-xs font-bold transition-all cursor-pointer ${!isHi ? 'bg-blood-600 text-white shadow-sm' : 'text-ink-400 hover:text-white'}`}>EN</button>
              <button onClick={() => setLanguage('HI')}
                className={`rounded-full px-2.5 py-1 text-xs font-bold transition-all cursor-pointer ${isHi ? 'bg-blood-600 text-white shadow-sm' : 'text-ink-400 hover:text-white'}`}>HI</button>
            </div>
